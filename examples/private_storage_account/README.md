...
In this example, we deploy an application gateway with a storage account that contains a static web application in a Storage Account, but the storage account is configured not to serve requests unless they originate from the Application Gateway's private subnet.

A side effect of this behavior is that it's no longer possible for us to load example files into the Storage Account from Terraform without knowing our external IP so that it can be added to the Storage Account's network rules! To fully exercise this example, you will need to upload an index.html to the bucket directly so that the page can be displayed. Failure to do this will result in a 502 Bad Gateway error from the Application Gateway as its liveness probes won't be able to find the default index.html site they expect in the Storage Account's $web container.

To achieve this:

1. Deploy this example with `terraform apply`
2. In the Azure console, locate the resource group and open the Storage Account
3. Navigate to Security + Networking > Networking
4. Under the Firewall heading, check the box that adds your client IP range to hte firewall.
5. Wait a few seconds for this to take effect, then upload an index.html file using Data Storage > Containers > $web > Upload. If you don't have an example index.html file already built, you may upload the three example files contained at `../public_storage_account/site_files/`.
6. Navigate back to Security + Networking > Networking and remove your client IP address.
7. Navigate to the primary web endpoint (*.web.core.windows.net) listed under Data Management > Static website, you should receive a 404 error message.
8. Navigate to the public URL for the Application Gateway (*.eastus2.cloudapp.azure.com) and verify the static website is displayed as expected.
...
| <a name="input_resource_names_map"></a> [resource\_names\_map](#input\_resource\_names\_map) | A map of key to resource\_name that will be used by tf-launch-module\_library-resource\_name to generate resource names | <pre>map(object({<br>    name       = string<br>    max_length = optional(number, 60)<br>  }))</pre> | <pre>{<br>  "app_gateway": {<br>    "max_length": 80,<br>    "name": "appgw"<br>  },<br>  "resource_group": {<br>    "max_length": 90,<br>    "name": "rg"<br>  },<br>  "storage_account": {<br>    "max_length": 24,<br>    "name": "sa"<br>  },<br>  "vnet": {<br>    "max_length": 80,<br>    "name": "vnet"<br>  }<br>}</pre> | no |
| <a name="input_environment_number"></a> [environment\_number](#input\_environment\_number) | The environment count for the respective environment. Defaults to 000. Increments in value of 1 | `string` | `"000"` | no |
| <a name="input_resource_number"></a> [resource\_number](#input\_resource\_number) | The resource count for the respective resource. Defaults to 000. Increments in value of 1 | `string` | `"000"` | no |
| <a name="input_logical_product_family"></a> [logical\_product\_family](#input\_logical\_product\_family) | (Required) Name of the product family for which the resource is created.<br>    Example: org\_name, department\_name. | `string` | `"launch"` | no |
| <a name="input_logical_product_service"></a> [logical\_product\_service](#input\_logical\_product\_service) | (Required) Name of the product service for which the resource is created.<br>    For example, backend, frontend, middleware etc. | `string` | `"appgw"` | no |
...
| <a name="output_storage_account_web_endpoint"></a> [storage\_account\_web\_endpoint](#output\_storage\_account\_web\_endpoint) | URL for the storage account's hosted website. |
